...
public class NodeImpl implements Node {
    ...
    public void appendLog(@Nonnull byte[] commandBytes) {
        ...
        context.taskExecutor().submit(() -> {
            ...
            doReplicateLog(); //send the commit log to major follower
    ...
    /**
     * @see NodeImpl#onReceiveRequestVoteRpc(RequestVoteRpcMessage)
     */
    ...
    private void doReplicateLog(GroupMember member, int maxEntries) {
        // sendAppendEntries to specified member.
        member.replicateNow();
        try {
     ...
     *
     * @param rpcMessage rpc message
     * @see NodeImpl#onReceiveAppendEntriesResult(AppendEntriesResultMessage)
     */
    @Subscribe
     ...
     * @see org.les.core.rpc.nio.AbstractHandler#channelRead(ChannelHandlerContext, Object)
     */
    @Subscribe
...
//        }
        ...
        if (result.isSuccess()) { // append Entries success?
            if (!member.isMajor()) {  // removing node // not in major
            ...
            //peer advance commit index if major of match index changed
            if (member.advanceReplicatingState(rpc.getLastEntryIndex())) { // change group state
