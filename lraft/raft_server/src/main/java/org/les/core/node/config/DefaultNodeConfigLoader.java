package org.les.core.node.config;
...
public class DefaultNodeConfigLoader implements NodeConfigLoader {
    ...
    public NodeConfig load(@Nonnull InputStream input) throws IOException {
        ...
        config.setMinElectionTimeout(getIntProperty(p, "election.timeout.min", 30000));
        config.setMaxElectionTimeout(getIntProperty(p, "election.timeout.max", 40000));

        config.setLogReplicationDelay(getIntProperty(p, "replication.delay", 0));
        config.setLogReplicationInterval(getIntProperty(p, "replication.interval", 10000));

        config.setLogReplicationReadTimeout(getIntProperty(p, "replication.timeout.read", 9000));
        config.setMaxReplicationEntries(getIntProperty(p, "replication.entries.max", -1));

        config.setSnapshotDataLength(getIntProperty(p, "snapshot.data.length", 1024));
        config.setMaxReplicationEntriesForNewNode(getIntProperty(p, "new-node.replication.entries.max", -1));

        config.setNewNodeMaxRound(getIntProperty(p, "new-node.round.max", 10));
        config.setNewNodeReadTimeout(getIntProperty(p, "new-node.timeout.read", 30000));
        config.setNewNodeAdvanceTimeout(getIntProperty(p, "new-node.timeout.advance", 30000));
